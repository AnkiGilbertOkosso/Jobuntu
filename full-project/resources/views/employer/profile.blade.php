...
@endphp
@php
    $social_links = json_decode($employer->social_links, true);
@endphp
    <div class="pt-14">
       <!-- component -->
        <div class="bg-white rounded-lg pb-8">
            ...
            <div class="w-full h-[250px]">
                <img src="{{ asset($employer->banner_image) }}" class="w-full object-cover h-full rounded-tl-lg rounded-tr-lg">
            </div>
            <div class="flex flex-col items-center ">
                <img src="{{ asset($employer->logo) }}" class="border-4 -mt-20 border-white w-32 h-32 rounded-full">
                <div class="flex items-center space-x-2 mt-5">
                    <p class="text-2xl font-semibold">{{ $employer->company_name }}</p>
                </div>
                <p class="text-xl text-gray-500">{{ $employer->map_location }}</p>
            </div>
            <div class="flex-1 flex flex-col items-center lg:items-end justify-end px-8 mt-2">
                <div class="flex items-center space-x-4 mt-2">
                    <a href="{{ route('jobs.create') }}" class="flex space-x-2 items-center bg-primary text-white px-4 py-2 rounded">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                            <path d="M8 9a3 3 0 100-6 3 3 0 000 6zM8 11a6 6 0 016 6H2a6 6 0 016-6zM16 7a1 1 0 10-2 0v1h-1a1 1 0 100 2h1v1a1 1 0 102 0v-1h1a1 1 0 100-2h-1V7z"></path>
                        </svg>
                        <span>Post a Job</span>
                    </a>
                    <a class="flex items-center space-x-2 justify-center border border-primary bg-transparent text-primary px-4 py-2 rounded">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="my-4 flex flex-col 2xl:flex-row space-y-4 2xl:space-y-0 2xl:space-x-4">
            <div class="w-full flex flex-col 2xl:w-1/3">
                <div class="flex-1 bg-white rounded-lg p-8">
                    ...
                    <ul class="mt-2 text-gray-700">
                        <li class="flex border-y py-2">
                            <span class="font-bold w-48">Company name:</span>
                            <span class="text-gray-700">{{ $employer->company_name }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-48">Year of Establishment:</span>
                            <span class="text-gray-700">{{ $employer->year_of_establishment->format('d M Y') }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-48">Joined:</span>
                            <span class="text-gray-700">{{ $employer->created_at->format('d M Y') }} ({{ $employer->created_at->diffForHumans() }})
                            </span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-48">Mobile:</span>
                            <span class="text-gray-700">{{ $employer->phone }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-48">Email:</span>
                            <span class="text-gray-700">{{ $employer->email }}</span>
                        </li>
                        <li class="flex border-b py-2">
                            <span class="font-bold w-48">Location:</span>
                            <span class="text-gray-700">{{ $employer->map_location }}</span>
                        </li>
                        <li class="flex items-center border-b py-2 space-x-2">
                            <span class="font-bold w-48">Social Links:</span>
                            <a href="{{ $social_links['facebook'] }}" title="Facebook">
                            ...
                            <a href="{{ $social_links['twitter'] }}" title="Twitter">
                            ...
                            <a href="{{ $social_links['linkedin'] }}" title="LinkedIn">
                            ...
                            <a href="{{ $social_links['instagram'] }}" title="Github">
                ...
                <div class="flex-1 bg-white rounded-lg p-8">
                    ...
                    <p class="mt-2 text-gray-700">{{ $employer->about_us }}</p>
                </div>
            </div>
